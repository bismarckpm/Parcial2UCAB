...
namespace Parcial2UCAB.Test
{
    public class UnitTest1
    {
        ...
        public UnitTest1()
        {
            ...
            _controller = new ActorController(_mockLogger, _mockActorDAO.Object);
        }
        ...
        public async Task CreateActor_Exception()
        {
            ...
            _mockActorDAO.Setup(x => x.CreateActor(request)).Throws(new Exception());

            await Assert.ThrowsAsync<Exception>(() => _controller.CreateActor(request));
        }
        ...
        public async Task CreateActor_Returns_Guid()
        {
            ...
            _mockActorDAO.Setup(x => x.CreateActor(request)).ReturnsAsync(expected);
        ...
        public async Task UpdateActor_Guid()
        {
            var request = new ActorRequest();
            var expected = new Guid();
            _mockActorDAO.Setup(x => x.UpdateActor(request)).ReturnsAsync(expected);
        ...
        {
            var request = new ActorRequest();
            _mockActorDAO.Setup(x => x.UpdateActor(request)).Throws(new Exception());

            await Assert.ThrowsAsync<Exception>(() => _controller.UpdateActor(request));
        }
        ...
        public async Task GetActor_Response()
        {
            ...
            _mockActorDAO.Setup(x => x.GetActor(actorId)).ReturnsAsync(expected);
        ...
        {
            var actorId = new Guid();
            _mockActorDAO.Setup(x => x.GetActor(actorId)).Throws(new Exception());
        ...
        {
            var actorId = new Guid();
            _mockActorDAO.Setup(x => x.GetActor(actorId)).ReturnsAsync((ActorResponse)null);
